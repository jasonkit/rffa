...
upgrade-db: ensure-db-ready
	$(RUN_TASK) sh -c "cd api-server && alembic upgrade $(upto)"

.PHONY: downgrade-db
downgrade-db: ensure-db-ready
	$(RUN_TASK) sh -c "cd api-server && alembic downgrade $(downto)"

.PHONY: upgrade-testdb
upgrade-testdb: ensure-testdb-ready
	$(RUN_TASK) sh -c "cd api-server && DATABASE_URL=\$$TEST_DATABASE_URL alembic upgrade $(upto)"

.PHONY: downgrade-testdb
downgrade-testdb: ensure-testdb-ready
	$(RUN_TASK) sh -c "cd api-server && DATABASE_URL=\$$TEST_DATABASE_URL alembic downgrade $(downto)"

.PHONY: create-migration
create-migration:
	${RUN_TASK} sh -c "cd api-server && alembic revision -m \"${name}\""
